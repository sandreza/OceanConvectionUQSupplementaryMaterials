...
# Parameteric dpendence on stratification

#=
if isempty(ARGS)
    const scale = 1
    println("looking at case " * string(scale))
else
    const scale = parse(Int, ARGS[1])
    println("looking at case "* string(scale) )
end
=#
const scale = temp_array[1]
println("looking at case "* string(scale) )
...
# Surface heat flux (positive values imply cooling)
@inline Q(x, y, t) = 100.0
Q_str = 100.0
...
field_writer = JLD2OutputWriter(model, fields; dir=base_dir, prefix="general_strat_" * string(scale) *"_fields",
                                init=init_save_parameters_and_bcs,
                                max_filesize=100GiB, interval=24hour, force=true, verbose=true)
...
profile_writer = JLD2OutputWriter(model, profiles; dir=base_dir, prefix = "general_strat_" * string(scale) * "_profiles",
                                  init=init_save_parameters_and_bcs,
...
wizard = TimeStepWizard(cfl=0.1, Δt=3.0, max_change=1.2, max_Δt=1.0)
...
while model.clock.time < end_time
    walltime = @elapsed time_step!(model; Nt=Ni, Δt = 2.0)
