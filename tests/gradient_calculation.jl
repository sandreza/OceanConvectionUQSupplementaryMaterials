...
##
fcalls = 100
bnm = [0.0760809666611145; 4.342473912404762; 2.1630355831002954; 5.57111619953263]
bnm = bestparams
lower = [0.0,0.0,0.0,0.0]
upper = [1.0, 8.0, 6.0, 5.0/0.3]
pswm = ParticleSwarm(lower = lower, upper = upper, n_particles = 30)
optimal_val = Optim.optimize(ℒ, bnm, pswm, Optim.Options(f_calls_limit = fcalls, store_trace = true))
(optimal_val.minimizer, optimal_val.minimum)

##
function gradientclosure(ℒ)
    return gradient(ℒ, params)
end
gg(params) = gradientclosure(ℒ)
Optim.optimize(ℒ, gg, bnm, LBFGS(), Optim.Options(f_calls_limit = fcalls, store_trace = true), inplace = false)
